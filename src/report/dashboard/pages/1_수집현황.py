...
st.title("① 수집 현황")
st.caption("TEP 다변량 스트림(52변수)을 SQLite에 누적 — 정상 위주 백필 구성")
...
c1.metric("총 적재 행", f"{n_total:,}")
c2.metric("정상(fault_id=0)", f"{n_normal:,}")
c3.metric("결함(IDV)", f"{n_fault:,}")
c4.metric("정상:결함", f"{ratio:.1f} : 1" if n_fault else "—")
